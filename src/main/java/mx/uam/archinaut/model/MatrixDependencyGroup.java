...
public class MatrixDependencyGroup {
	...
	// A hashmap that contains dependencies the string is the type the integer is
	...
	/**
	 * Add a new dependency. In case this type already existed, the occurrences are
	 * incremented
	 * 
	...
	public static MatrixDependencyGroup getDependencies(MatrixElement source, MatrixElement destination) {
		...
		if (groups.size() > 1) {
			...
			logger.trace("MatrixDependencyGroup.getDependencies: found " + groups.size() + " between " + source
					+ " and " + destination);
		}
		if (groups.size() == 1)
			return groups.get(0);
	...
	/**
	 * Get dependencies from a source element with respect to an array of possible
	 * targets the targets may be groups, in that case, the MatrixDependencyGroups
	 * that are returned aggegate dependencies of dependencies between the source
	 * element and children of the target groups
	 * 
	 * @param source
	 * @param possibleTargets
	 * @return
	 */
	public static List<MatrixDependencyGroup> getDependencies(MatrixElement source,
			List<MatrixElement> possibleTargets) {

		// We need a map as a destination may occur in more than one dependency, to
		// aggregate numbers
		...
		if (!(source instanceof MatrixElementGroup)) {
			...
			for (MatrixDependencyGroup currentDependencyGroup : source.getDependencies()) {
				...
				if (possibleTargets.contains(newDestination)) {
					...
					if (newDestination != null) {
						...
						AggregateMatrixDependencyGroup newDependencies = (AggregateMatrixDependencyGroup) depsMap
								.get(newDestination);
			...
			// All the children of this group will be grouped under the same source, which
			// is the group
