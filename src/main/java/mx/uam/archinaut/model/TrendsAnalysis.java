...
public class TrendsAnalysis implements Serializable {
	...
	public enum Trend {
		HIGH_DEGRADING, DEGRADING, STABLE, IMPROVING, HIGH_IMPROVING, UNKNOWN
	}
	...
	public TrendsAnalysis(List<DesignStructureMatrix> matrixes) {
		...
		for (MatrixElement element : currentMatrix.getElements()) {
			...
			for (int column = 0; column < matrixes.size() - 1; column++) {
				...
				// If the element was not present in a previous snapshot, thisElement will be
				// null
	...
	/**
	 * Returns the values of the metric of an element across all snapshots if an
	 * element is not present in a particular snapshot, the value for that snapshot
	...
	/**
	 * Return the total number of elements which is given by the first snapshot
	 * (deleted elements will not appear)
	 * 
	 * @return
	 */
	...
	/**
	 * Get the maximum value of a given metric across all snapshots
	 * 
	 * @param type
	 * @return
	 */
	...
	/**
	 * Calculate a difference in the value of a metric for an element between two
	 * indexes first index must be bigger than the second index as the snapshots go
	...
	public int getDelta(String elementFullName, ElementMetric type, int firstIndex, int secondIndex) {
		...
		if (secondIndex >= matrixes.size() || firstIndex >= matrixes.size())
			throw new IndexOutOfBoundsException("Index bigger than matrix size first: " + firstIndex + " second: "
					+ secondIndex + " size:" + matrixes.size());
		...
		if (firstIndex < secondIndex)
			throw new IndexOutOfBoundsException(
					"first index smaller than second index first: " + firstIndex + " second: " + secondIndex);
	...
	/**
	 * Calculates the maximum and minimum values of deltas across all snapshots for
	 * a given metric
	 * 
	 * @param type
	...
	/**
	 * Return the total decreases and increases for a given snapshot, a metric and a
	 * boolean indicating if deltas should be used
	 * 
	 * @param index
	 * @param type
	 * @param deltas
	...
	public Trend getTrendFor(String elementFullName, ElementMetric type) {
		...
		// logger.info("Slope for"+elementFullName+" = "+slope+" intercept =
		// "+intercept);

		if (slope < -30)
			return Trend.HIGH_IMPROVING;
		if (slope >= -30 && slope < -10)
	...
	/**
	 * Creates a report for the different metrics and snapshot, separating the
	...
	/**
	 * Return list of names of elements based on their status: PRESENT: elements
	 * that are present in the snapshot RENAMED: elements that are present in this
	 * snapshot but renamed in the next one DELETED: elements that are present in
	 * this snapshot but deleted in the next one
	 * 
	 * @param index
	 * @return
	 */
